...
function Name(props) {
    ...
    useEffect(function sortName(ip='name',e){
            const sorted = [...response.list].sort((a,b)=>
            a[ip].toLowerCase() > b[ip].toLowerCase() ? 1 : -1
            )
            setData(sorted)
			setisActive(true)
	},[])	
	...
	return (
		<div className="text-center mt-50">
				<div>
					...
					<Link to='/name'><button data-testid="route-name"  className={`${isActive ? "select": "outlined"}`}  type="button">Name</button></Link>
